...
import { Plus, Pencil, Trash2, UserCheck, Loader2, Search, Eye, Phone, User } from "lucide-react";
...
export function VisitsView() {
  ...
  const [visitorModal, setVisitorModal] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto">
      ...
      {/* Dialog: تفاصيل الزائر */}
      <Dialog open={!!visitorModal} onOpenChange={(o) => !o && setVisitorModal(null)}>
        <DialogContent className="max-w-sm">
          ...
          {visitorModal && (
            <div className="space-y-3 py-2">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                ...
                <div>
                  <div className="font-semibold text-lg">{visitorModal.name || "—"}</div>
                  <div className="text-xs text-muted-foreground font-mono">{visitorModal.code || "—"}</div>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between p-3 rounded-lg border">
                  ...
                  <span className="font-medium" dir="ltr">
                    {visitorModal.phone || "غير متوفر"}
                  </span>
                </div>
              </div>
              {visitorModal.phone && (
                <a href={`tel:${visitorModal.phone}`} dir="ltr" className="block">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setVisitorModal(null)}>إغلاق</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
